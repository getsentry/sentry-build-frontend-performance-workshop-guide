...
description: Set up alerts, dashboards, and AI-powered debugging to operate your app in production with confidence
---

import { Steps } from '@astrojs/starlight/components';
import ScaledImage from '../../components/ScaledImage.astro';

You've instrumented Sentry, fixed Web Vitals issues, optimized backend performance, and debugged UX problems with Session Replay. Now it's time to prepare for production: setting up alerts to catch issues fast, creating dashboards for ongoing monitoring, and leveraging **Seer** (Sentry's AI debugger) for automated root cause analysis.

Throughout this workshop, you've been fixing issues manually. Now that you know what "good" performance looks like, you can set up automated systems to protect these improvements and catch regressions before they impact users.

## Learning Objectives

By the end of this module, you will:

- Configure alerts to catch performance regressions and errors before users notice
- Build dashboards to monitor key metrics across your application
...
Throughout this workshop, you've been manually investigating issues in Sentry. In production, you need automated systems to notify you when things go wrong and help you debug faster. Let's set up:

1. **Alerts** - Get notified when errors spike or performance degrades
2. **Dashboards** - Monitor key metrics at a glance
3. **GitHub Integration** - Enable AI-powered debugging with Seer

Let's start with the basics: alerts and monitoring.

### Setting Up Alert Rules

Now that you've optimized your application, you know what "good" performance looks like. Let's set up alerts to catch regressions and protect the improvements you've made.

**Why set alerts now instead of in Module 1?**

- In Module 1, you didn't know what thresholds to set (Is 2s good? Bad? Depends on your app!)
- Now you have a baseline: LCP < 2.5s, API responses < 200ms, no N+1 queries
- These alerts will notify you if performance degrades from this optimized state

With all the data flowing through Sentry, let's set up alerts based on your improved performance baseline.
...
1. **Create an Error Rate Alert (Frontend)**
   ...
   A 1% error rate means 1 out of every 100 users hits an issue - significant for e-commerce conversion rates. Throughout this workshop, you've fixed validation errors, N+1 queries, and UX issues that caused errors. This alert catches regressions before they impact customers.
   :::
...
2. **Create a Web Vitals Alert (LCP)**
   ...
   :::note[Why This Threshold?]
   In Module 2, you optimized LCP from > 4s to < 2.5s by fixing image loading and layout shifts. This alert catches regressions to those improvements. LCP > 2.5s is "poor" and costs ~7% of conversions - critical during holiday traffic.
   :::
...
3. **Create a Backend Performance Alert**
   ...
   :::note[Why This Threshold?]
   In Module 3, you fixed N+1 queries that caused 2.5s API responses. After optimization, your sale API responds in ~150ms. This alert (1000ms threshold) catches any new N+1 patterns or database issues before they degrade user experience.
   :::
...
Alerts notify you when things break. Dashboards help you monitor trends and spot issues before they trigger alerts.
...
1. **Create a custom dashboard**
   ...
   - Name it: "E-commerce Performance Overview"

2. **Add Web Vitals widget**

   Add a widget to track your Core Web Vitals:
   - **Widget Type**: Line Chart
   - **Metric**: `p75(measurements.lcp)`, `p75(measurements.cls)`, `p75(measurements.fcp)`
   - **Group By**: `transaction`
   - **Time Range**: Last 7 days

   This shows which pages have poor Web Vitals over time.

   {/* TODO: Add screenshot of Web Vitals dashboard widget */}

3. **Add API Performance widget**

   Track your backend API performance:
   - **Widget Type**: Table
   - **Metric**: `p95(transaction.duration)`
   - **Filter**: `transaction.op:http.server`
   - **Sort By**: p95 (descending)

   This highlights your slowest API endpoints.

4. **Add Error Rate widget**
   ...
   Spikes in error rate often indicate deployment issues.

5. **Add Throughput widget**

   Track your application traffic:
   - **Widget Type**: Line Chart
   - **Metric**: `count()`
   - **Group By**: `transaction.op`
   - **Time Range**: Last 7 days

   Understand which parts of your app get the most traffic.
...
- Review dashboards weekly during team meetings
- Create separate dashboards for frontend and backend
- Share dashboards with stakeholders to show performance improvements
- Set dashboard as your organization's homepage for visibility
  :::
...
Seer is Sentry's AI debugging assistant that helps you fix issues faster:

- **Analyzes issues**: Understands traces, errors, and user impact
- **Finds root causes**: Identifies N+1 queries, missing indexes, inefficient code patterns
- **Suggests fixes**: Provides code-level recommendations specific to your codebase
- **Opens PRs**: Automatically creates pull requests with fixes (optional)
- **Learns from your code**: Uses GitHub integration for context-aware suggestions

With GitHub connected, Seer can read your code, understand your architecture, and provide highly specific recommendations instead of generic advice.
...
1. **Find a poor LCP transaction**
   - Navigate to **Performance** → **Web Vitals** → **LCP**
...
1. **Weekly performance review with Seer**
   ...
   - Go to **Performance** → Sort by p95 duration
...
3. **Apply the fix**
   ...
   ```tsx
   // ❌ Current (crashes on undefined)
   const total = cartItems.reduce((sum, item) => sum + item.price, 0);

   // ✅ Suggested (handles missing prices)
...
- **Alerts catch issues fast** - Configure error rate, Web Vitals, and backend performance alerts
- **Dashboards provide visibility** - Monitor trends and spot issues before they escalate
- **GitHub integration enables AI** - Seer needs code access for context-aware suggestions
- **Seer provides root cause analysis** for performance issues and errors
- **AI-suggested fixes** are specific to your codebase, not generic patterns
- **Automated PRs** save time but still need human review
- **Trust but verify** - Seer is a powerful tool, not a replacement for developer judgment

## Workshop Complete! 🎉

Congratulations! You've built a comprehensive frontend performance monitoring system:

✅ **Module 1:** Instrumented Sentry for performance & error tracking
✅ **Module 2:** Fixed Web Vitals issues (LCP, CLS, FCP)
✅ **Module 3:** Traced frontend slowness to backend N+1 queries
✅ **Module 4:** Used Session Replay to fix UX issues hurting conversions
✅ **Module 5:** Set up alerts, dashboards, and AI-powered debugging with Seer

### Your Application is Now Ready For:

- **High traffic periods** (Black Friday, holiday sales)
- **Fast issue detection** with real-time alerts and dashboards
- **Quick debugging** with Session Replay and distributed tracing
- **Proactive optimization** with Seer AI suggestions
- **Team visibility** with shared performance dashboards
- **Confident deployments** with comprehensive monitoring
...
1. Review your performance dashboard weekly
2. Investigate alerts as they fire
3. Ask Seer about slow transactions
4. Watch Session Replays for UX issues
5. Iterate on performance optimizations based on data
...
**Thank you for participating!** Your e-commerce application is now instrumented, optimized, and ready to handle holiday traffic with confidence. Keep monitoring, keep optimizing, and keep shipping! 🚀
