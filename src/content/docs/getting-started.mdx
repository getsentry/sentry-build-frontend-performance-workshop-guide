...
4. **Import instrumentation in `main.tsx`**
   ...
   ```typescript title="main.tsx" {2, 5, 10-12}
...
:::tip[Track User Context with Sentry.setUser()]
To associate errors and performance data with specific users, call `Sentry.setUser()` when users authenticate. This helps you understand which users are affected by issues.

**Where to add it:** In your `AuthContext` (`apps/web/src/context/AuthContext.tsx`), add these three changes:

```typescript title="AuthContext.tsx" ins={2, 8-11, 16}
// 1. Add import at the top
import * as Sentry from '@sentry/react';

// 2. Inside login function (after successful login):
...
// 3. Inside logout function:
localStorage.removeItem('user');

Sentry.setUser(null);
```

This context will appear in all errors, performance transactions, and session replays, making it easier to debug user-specific issues.
:::
