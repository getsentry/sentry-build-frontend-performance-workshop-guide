...
```bash title="Generating sample usage data"
pnpm generate:usage
```
...
:::tip
Keep your dev server running (`pnpm dev`) while this script executes. It should complete in 30-60 seconds and generate transactions you can view in Sentry.
:::
...
To learn more about Web Vitals, check out our <LinkButton href="https://docs.sentry.io/product/performance/web-vitals/" target="_blank" variant="minimal">documentation.</LinkButton>
...
<div className="relative">
  <img
    ...
    // ❌ No loading="eager" or "lazy"
    // ❌ No decoding="async"
    // ❌ Parent has no aspect-ratio - container collapses until image loads
  />
</div>
```
...
function Sale() {
  ...
  if (loading) {
    return null; // ❌ Nothing renders! Completely blank page!
  }

  if (error) {
    return <div>{/* Error UI */}</div>; // ❌ Also no header!
  }

  return (
    <>
      <Header /> {/* ❌ Header only renders AFTER data loads */}
